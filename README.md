...
## Configuration & Secrets (vaulted)

Do NOT place passwords, tokens, hostnames, or IP addresses in tracked files or
README examples. All sensitive runtime configuration must live in the vaulted
env file at `~/.ansible/conf/env.yml`. Use the repository Jinja templates
(`templates/*.j2`) as examples and render them via the provided playbook
instead of copying literal values into docs.

Create a vault password file and an encrypted env file like this:

```bash
mkdir -p ~/.ansible/conf
read -s -p "Enter Ansible Vault password: " VAULT_PASS
printf "%s\n" "$VAULT_PASS" > ~/.ansible/conf/.vaultpass.txt
chmod 600 ~/.ansible/conf/.vaultpass.txt
echo
ANSIBLE_VAULT_PASSWORD_FILE=~/.ansible/conf/.vaultpass.txt ansible-vault create ~/.ansible/conf/env.yml
```

To view or edit the vaulted file without leaving plaintext on disk:

```bash
ANSIBLE_VAULT_PASSWORD_FILE=~/.ansible/conf/.vaultpass.txt ansible-vault view ~/.ansible/conf/env.yml
ANSIBLE_VAULT_PASSWORD_FILE=~/.ansible/conf/.vaultpass.txt ansible-vault edit ~/.ansible/conf/env.yml
```
...
**Issue 2: "Cannot reach aap-26 via SSH"**
...
./rhis_install.sh --non-interactive --menu-choice 7
```
...
**Runtime Note:** `rhis_install.sh` regenerates `host_vars/*.yml` during config-as-code startup. Treat these files as generated runtime artifacts.
