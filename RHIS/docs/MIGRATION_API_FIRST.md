...
- Converted many `hammer` call sites in `MRHIS.sh` and `MRHIS.py` to use the wrapper (subnet create, lifecycle envs, content view attach/publish, compute resource, activation keys, etc.).
...
1. Finish converting remaining `MRHIS.sh` raw hammer lines (if any).
