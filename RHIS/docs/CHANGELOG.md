...
## 2026-03-31 — Hotfixes: MRHIS troubleshooting

### 2026-03-31 — Hotfixes applied during Satellite post-install troubleshooting

- **Area:** `container/vars/host_vars/satellite_content_profile.yml`, `container/roles/rhis-builder-satellite/roles/lifecycle_environments/tasks/main.yml`, `MRHIS.sh`
- **Summary:** Patched recursive templating in `satellite_content_profile.yml`, added missing `label:` fields to lifecycle environment definitions, enabled debug output for the lifecycle_environments tasks and added managed hotfix functions to `MRHIS.sh` to apply these fixes into the running provisioner container.
- **Reason:** These hotfixes surface module errors, ensure schema compatibility with the `redhat.satellite` Ansible modules, and allow the installer to reapply fixes automatically to the provisioner container when it is created or restarted.

### 2026-03-28 — Roles folder consolidation + rhis_installer.py CLI alignment

- **Area:** `rhis_installer.py`, `README.md`, `CHECKLIST.md`, `CHANGELOG.md`, `.PROMPTS_REFERENCE.md`, `host_vars/README.md`, `inventory/README.md`, `docs/README.md`, `container/roles/host_vars/README.md`, `container/vars/external_inventory/README.md`, `container/vars/host_vars/README.md`
- **Summary:**
  ...
  - Expanded `rhis_installer.py` `main()` argparse to include all CLI flags from `MRHIS.sh`: `--non-interactive`, `--env-file`, `--inventory`, `--inventory-growth`, `--container-config-only`, `--satellite`, `--idm`, `--aap`, `--attach-consoles`, `--status`, `--reconfigure`, `--test[=fast|full]`, `--demo`/`--DEMO`, `--demokill`/`--DEMOKILL`, `--validate`/`--preflight`, `--generate-env`.
  ...
  - Bulk-replaced all `rhis_install.sh` → `MRHIS.sh` and `rhis_install.py` → `rhis_installer.py` references across all `*.md` documentation files.
...
- **Area:** `MRHIS.sh`, `rhis_installer.py`, `README.md`, `.PROMPTS_REFERENCE.md`, `CHECKLIST.md`, `host_vars/README.md`, `inventory/README.md`, `docs/README.md`, `QUICK_RECOVERY.md`
- **Summary:**
...
### 2026-03-24 10:50:00 MDT — Merge headless helper files into MRHIS.sh

- **Area:** `MRHIS.sh` (new functions), `rhis-headless-validate.sh` (removed), `rhis-headless.env.template` (removed)
- **Summary:**
  - Merged standalone `rhis-headless-validate.sh` into `MRHIS.sh` as `validate_headless_config()` function.
  - Merged `rhis-headless.env.template` into `MRHIS.sh` as `generate_env_template()` function (heredoc).
  ...
  - Removed `rhis-headless-validate.sh` and `rhis-headless.env.template` from repository (functionality now in `MRHIS.sh`).
...
- **Area:** `README.md`, `CHECKLIST.md`, `docs/README.md`, `host_vars/README.md`, `inventory/README.md`
- **Summary:**
...
- **Reason:** Provide durable, per-run operational logs for troubleshooting and auditability of each `MRHIS.sh` execution.
